  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <main className="ml-[85px] lg:ml-56 flex-1 p-4">
        ...
        {viewCampaign ? (
          <CampaignDetails campaign={viewCampaign} onClose={() => setViewCampaign(null)} />
        ) : (
          <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 items-start lg:px-16 py-5">
                ...
                <div className="bg-white border border-rose-200 rounded-xl p-6 text-sm text-rose-700 flex items-start justify-between gap-4">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <div className="mt-1">Failed to fetch campaigns.</div>
                    </div>
                  </div>
                  <button
                    className="shrink-0 px-3 py-2 rounded-lg border border-rose-200 text-rose-700 hover:bg-rose-50 text-sm font-medium" onClick={() => window.location.reload()}>Retry</button>
                </div>
              </div>
            )}
              ...
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/10 px-4">
                <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
                  ...
                  <p className="mt-2 text-sm text-gray-600">
                    ...
                    <span className="font-medium text-gray-900">{campaignToDelete?.campaignName}</span>? This action cannot be undone.
                  </p>
                  <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                    <button
                      ...
                      onClick={async () => {
                        try {
                          deleteCampaignMutation.mutate(campaignToDelete._id);
                        } catch (err) {
            ...
            {!isLoading &&
              ...
              campaigns.map((campaign) => {
                const targetTotal = Number(campaign.targetContactsCount || 0);
                const responses = Number(campaign.responses || 0);
                const conversions = Number(campaign.conversions || 0);
                const responseRate = targetTotal > 0 ? Math.round((responses / targetTotal) * 100) : 0;
                const progressPercent = targetTotal > 0 ? Math.min(100, Math.round((responses / targetTotal) * 100)) : 0;
                ...
                return (
                  <div
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter" || e.key === " ") openDetails(campaign);
                    }}
                    className="rounded-2xl bg-white p-5 shadow-[0_12px_30px_rgba(17,24,39,0.08)] ring-1 ring-black/5 h-[-webkit-fill-available] cursor-pointer hover:ring-black/10 hover:shadow-[0_16px_36px_rgba(17,24,39,0.10)] transition"
                  >
                    <div
                    ...
                    >
                      <div>
                        <h3 className="text-lg font-semibold text-gray-900">{campaign.campaignName || "Untitled Campaign"}</h3>
                        <span
                          className={`mt-2 inline-flex rounded-md border px-2 py-0.5 text-xs font-medium ${statusColors[statusKey] || statusColors.other
                            }`}
                        >
                          {statusKey}
                        </span>
                    ...
                    <div className="mt-5">
                      ...
                      <div className="mt-3 h-2 w-full rounded-full bg-gray-100 overflow-hidden">
                        <div className="h-full rounded-full bg-gray-300" style={{ width: `${progressPercent}%` }} />
                      </div>
                    </div>

                    <div className="mt-5 grid grid-cols-3 text-center">
                      <div>
                        <div className="text-xl font-bold text-indigo-600">{responses}</div>
                        <div className="text-xs text-gray-500">Responses</div>
                      </div>
                      <div>
                        <div className="text-xl font-bold text-emerald-600">{conversions}</div>
                        <div className="text-xs text-gray-500">Conversions</div>
                      </div>
                      <div>
                        <div className="text-xl font-bold text-violet-600">{responseRate}%</div>
